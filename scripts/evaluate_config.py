...
from src.preprocessing.file_utils import Metadata, load_satellite
...
class ESDDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self, seed=1024):
        ...
        # Process the train tiles
        for tile in train_tiles:
            # Lload and preprocess the data for all satellite types
...
class ESDSegmentation(pl.LightningModule):
    ...
    def __init__(
        ...
        self.per_class_acc = Accuracy(
            task="multiclass", num_classes=out_channels, average=None
        )

        # Area Under Curve
        self.avg_AUC = MulticlassAUROC(
            num_classes=out_channels, average="macro", thresholds=None
        )
        self.per_class_AUC = MulticlassAUROC(
            num_classes=out_channels, average="none", thresholds=None
        )

        # F1 Score
        self.avg_F1Score = MulticlassF1Score(num_classes=out_channels)
        self.per_class_F1Score = MulticlassF1Score(
