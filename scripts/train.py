...
def train(options: ESDConfig):
    ...
    # If GPU isn't avaliable use the CPU
   
    if torch.cuda.is_available():
        print('cudav',torch.version.cuda)
    ...
    callbacks = [
        ModelCheckpoint(
            dirpath=ROOT / "model_checkpoints" / options.model_type,
    ...
    if options.checkpoint_file != "" and options.load_from_check_point:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_file", type=str, default="", help=".")
    
    parser.add_argument("--lambda_l1", type=float, help="The learning rate for training model", default=config.lambda_l1)
    ...
    parser.add_argument("--num_workers", help="Number of workers for dataloader", type=int, default=config.num_workers)
